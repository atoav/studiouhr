...
class Base(object):
    ...
    def __init__(self, x, y, z=0, color=(0.,0.,0.,1.), stroke=0, rotation=0):
        try:
            ...
            # this inits x,y and loc as well
...
class Line(Base):
    def __init__(self, a=(0,0), b=(0,0), z=0, color=(0,0,0,1), stroke=0, style=0):
        """ Draws a basic line given the begining and end point (tuples), color (tuple) and stroke
            (thickness of line)
            Line( x,y, a=(1,1), b=(100,100), z=0, color=(0.2,0,0,1), stroke=10, rotation=45)
        """
        ...
        self.a = vector.Vector(a[0], a[1])
        self.b = vector.Vector(b[0], b[1])
        print(b)
        self.rect = Rect(x, y, w, h)
        self.style = style
        Base.__init__(self, x, y, z,color,stroke,rotation)
...
##            glDisable(GL_LINE_STIPPLE)
        ...
        glBegin(GL_LINES)
        glVertex2i(int(self.a.x), int(self.a.y)) # draw pixel points
        glVertex2i(int(self.b.x), int(self.b.y))
        glEnd()
